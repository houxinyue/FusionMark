"""
PyMuPDF + LangExtract 融合 Pipeline
======================================
替代 MinerU + LangExtract 方案，直接用 PyMuPDF 提取文字坐标

流程：
1. PyMuPDF 提取文字坐标 → JSON (替代 MinerU layout.json)
2. PyMuPDF 提取纯文本 → LangExtract
3. LangExtract 提取实体
4. 文本匹配 → 坐标查找
5. PyMuPDF 渲染高亮
"""
...
def run_langextract(text: str) -> List[lx.data.Extraction]:
    """
    使用 LangExtract 从文本提取信息
    """
    print("=" * 60)
...
def build_span_index_from_pymupdf(extractor: PyMuPDFTextExtractor) -> Dict[str, List[PositionMatch]]:
    """
    使用 PyMuPDF 建立文本到位置的索引（替代 MinerU layout.json）
    """
    print("\n" + "=" * 60)
    print("Step 2: 使用 PyMuPDF 构建位置索引")
    print("=" * 60)
...
def main():
    """完整流程演示：PyMuPDF + LangExtract"""
    print("\n" + "=" * 70)
    print("PyMuPDF + LangExtract 融合演示（替代 MinerU）")
    print("=" * 70)
    ...
    if not os.getenv("DS_API_KEY"):
        print("❌ 未配置 DS_API_KEY 环境变量")
        return
    
    OUTPUT_DIR = Path("pymupdf_output")
    OUTPUT_DIR.mkdir(exist_ok=True)
    # 使用 PyMuPDF 提取器
    with PyMuPDFTextExtractor(test_pdf) as extractor:
        # 1. 提取纯文本
        print(f"\n📄 提取纯文本...")
        ...
        # 2. LangExtract 提取
        extractions = run_langextract(text)
        
        if not extractions:
            print("❌ 没有提取到任何信息")
            return
        
        # 3. 使用 PyMuPDF 构建位置索引（替代 MinerU layout.json）
        index = build_span_index_from_pymupdf(extractor)
        ...
        # 5. 渲染 PDF
        output_pdf = str(OUTPUT_DIR / "pymupdf_fusion_highlighted.pdf")
        render_highlights(test_pdf, highlights, output_pdf)
